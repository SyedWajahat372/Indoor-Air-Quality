...
public partial class DashboardPage : ContentPage
{
    ...
    // Gauge Drawables
    private readonly AQIGaugeDrawable aqiDrawable = new AQIGaugeDrawable { MaxValue = 100f, StatusText = "WAITING AI", Value = 0f };
    ...
    private async Task PredictAirQuality(SensorModel sensor)
    {
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                aqiDrawable.Value = 0f;
                aqiDrawable.StatusText = "WAITING AI";
                aqiDrawable.PrimaryColor = Color.FromArgb("#64748B");
            ...
            if (result != null && result.air_quality != null)
            {
                ...
                // 1. Update Labels strictly from AI Model Output
                PredictionLabel.Text = label.ToUpper();
                ConfidenceLabel.Text = $"Confidence : {confidence:F1}%";
                StatusLabel.Text = status;

                // 2. Update Gauge strictly from AI Model Output
                ...
                {
                    aqiDrawable.PrimaryColor = Color.FromArgb("#EF4444"); // Red
                }
                else if (labelUpper.Contains("MODERATE") || labelUpper.Contains("WARNING") || labelUpper.Contains("FAIR"))
                {
                    aqiDrawable.PrimaryColor = Color.FromArgb("#F59E0B"); // Amber
                }
                else
                {
                    aqiDrawable.PrimaryColor = Color.FromArgb("#10B981"); // Green
                }
...
print("Updated DashboardPage.xaml.cs to be 100% AI Model driven with 9 PM offline state!")
