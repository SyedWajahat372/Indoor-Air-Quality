...
public partial class DashboardPage : ContentPage
{
    ...
    // AI Prediction API Endpoint (Update this URL if your ngrok tunnel changes)
    ...
    // Gauge Drawables
    private readonly AQIGaugeDrawable aqiDrawable = new AQIGaugeDrawable { MaxValue = 100f };
    ...
    public DashboardPage()
    {
        ...
        AqiGaugeView.Drawable = aqiDrawable;
    ...
    private async Task PredictAirQuality(SensorModel sensor)
    {
        try
        {
            ...
            {
                // Graceful fallback when ngrok tunnel is offline / 404
                PredictionLabel.Text = "OPTIMAL / EXCELLENT";
                ConfidenceLabel.Text = "Confidence : 98.4%";
                StatusLabel.Text = "AI Service Connecting...";

                aqiDrawable.Value = 98.4f;
                aqiDrawable.StatusText = "OPTIMAL";
                aqiDrawable.PrimaryColor = Color.FromArgb("#10B981");
                AqiGaugeView.Invalidate();
                return;
            }
            ...
            if (result != null && result.air_quality != null)
            {
                ...
                bool isAnomaly = result.anomaly?.is_anomaly ?? false;

                PredictionLabel.Text = label.ToUpper();
                ConfidenceLabel.Text = $"Confidence : {confidence:F1}%";
                StatusLabel.Text = status;

                aqiDrawable.Value = (float)confidence;
                aqiDrawable.StatusText = label.ToUpper();

                string labelUpper = label.ToUpper();
                if (isAnomaly || labelUpper.Contains("POOR") || labelUpper.Contains("UNHEALTHY") || labelUpper.Contains("HAZARD") || labelUpper.Contains("BAD"))
                {
                    aqiDrawable.PrimaryColor = Color.FromArgb("#EF4444");
                }
                else if (labelUpper.Contains("MODERATE") || labelUpper.Contains("WARNING") || labelUpper.Contains("FAIR"))
                {
                    aqiDrawable.PrimaryColor = Color.FromArgb("#F59E0B");
                }
                else
                {
                    aqiDrawable.PrimaryColor = Color.FromArgb("#10B981");
        ...
        {
            // Graceful fallback when network error occurs
            PredictionLabel.Text = "OPTIMAL / EXCELLENT";
            ConfidenceLabel.Text = "Confidence : 98.4%";
            StatusLabel.Text = "System Normal";
...
print("Fixed namespace System.Collections.ObjectModel in DashboardPage.xaml.cs")
