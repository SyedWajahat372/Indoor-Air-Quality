...
public partial class DashboardPage : ContentPage
{
    ...
    // AI Prediction API Endpoint (Live ngrok Server)
    ...
    private async Task PredictAirQuality(SensorModel sensor)
    {
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                PredictionLabel.Text = $"HTTP {(int)response.StatusCode} ERROR";
                ConfidenceLabel.Text = "Confidence : 0%";
                StatusLabel.Text = errorContent.Length > 80 ? errorContent.Substring(0, 80) : errorContent;
                return;
            }
            ...
            if (result != null)
            {
                string label = result.AQ_label ?? "UNKNOWN";
                double confidence = result.confidence_pct;
                bool isAnomaly = result.is_anomaly;

                // 1. Update Labels strictly from Live AI Model Output
                PredictionLabel.Text = label.ToUpper();
                ConfidenceLabel.Text = $"Confidence : {confidence:F1}%";
                StatusLabel.Text = isAnomaly ? $"Anomaly Detected (Score: {result.anomaly_score:F3})" : "System Normal • No Anomaly Detected";
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task LoadSensorData()
    {
        try
        {
            ...
            if (data != null && data.Count > 0)
            {
                ...
                // Filter out SPS30 Hardware Glitch Values (When Red Wire is Disconnected)
                if (latest.pm25 > 1000.0) latest.pm25 = 0.0;
                if (latest.pm1 > 1000.0) latest.pm1 = 0.0;
                if (latest.pm4 > 1000.0) latest.pm4 = 0.0;
                if (latest.pm10 > 1000.0) latest.pm10 = 0.0;
                ...
                // 4. Request Live AI Model Prediction
...
print("Added PM glitch safeguard filter in DashboardPage.xaml.cs!")
