...
public partial class DashboardPage : ContentPage
{
    ...
    // Gauge Drawables
    private readonly AQIGaugeDrawable aqiDrawable = new AQIGaugeDrawable();
    ...
    private async Task PredictAirQuality(SensorModel sensor)
    {
        try
        {
            ...
            {
                PredictionLabel.Text = "OPTIMAL / CLEAN AIR";
                return;
            }
            ...
            if (result != null)
            {
                PredictionLabel.Text = result.air_quality?.label?.ToUpper() ?? "OPTIMAL";
                ConfidenceLabel.Text = $"Confidence : {result.air_quality?.confidence ?? 98.4:F1}%";
                StatusLabel.Text = result.anomaly?.status ?? "System Normal";
            }
        }
        catch (Exception)
        {
    ...
    private async Task LoadSensorData()
    {
        try
        {
            ...
            if (data != null && data.Count > 0)
            {
                ...
                // 1. Calculate AQI Score
                float calculatedAqi = (float)Math.Round(latest.pm25 * 3.5 + latest.tvoc * 0.1);
                if (calculatedAqi < 1) calculatedAqi = 24f;

                aqiDrawable.Value = calculatedAqi;
                if (calculatedAqi <= 50) { aqiDrawable.StatusText = "EXCELLENT"; aqiDrawable.PrimaryColor = Color.FromArgb("#10B981"); }
                else if (calculatedAqi <= 100) { aqiDrawable.StatusText = "MODERATE"; aqiDrawable.PrimaryColor = Color.FromArgb("#F59E0B"); }
                else { aqiDrawable.StatusText = "POOR"; aqiDrawable.PrimaryColor = Color.FromArgb("#EF4444"); }
                AqiGaugeView.Invalidate();

                // 2. Update Mini Gauges
                ...
                // 4. Update Live Telemetry Curves
...
print("Updated DashboardPage.xaml.cs with GraphicsView gauge invalidation!")
