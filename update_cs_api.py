...
public partial class DashboardPage : ContentPage
{
    ...
    private async Task PredictAirQuality(SensorModel sensor)
    {
        try
        {
            ...
            {
                PredictionLabel.Text = $"API ERROR ({response.StatusCode})";
                return;
            }
            ...
            if (result != null && result.air_quality != null)
            {
                ...
                // 1. Update Labels directly from AI API Output
                PredictionLabel.Text = label.ToUpper();
                ConfidenceLabel.Text = $"Confidence : {confidence:F1}%";
                StatusLabel.Text = status;

                // 2. Update Gauge Value & Status directly from AI API Output
                ...
                {
                    aqiDrawable.PrimaryColor = Color.FromArgb("#EF4444"); // Red for Anomaly / Poor
                }
                else if (labelUpper.Contains("MODERATE") || labelUpper.Contains("WARNING") || labelUpper.Contains("FAIR"))
                {
                    aqiDrawable.PrimaryColor = Color.FromArgb("#F59E0B"); // Amber for Moderate
                }
                else
                {
                    aqiDrawable.PrimaryColor = Color.FromArgb("#10B981"); // Green for Optimal / Good
                }
        ...
        {
            PredictionLabel.Text = "API CONNECTION ERROR";
        }
    }
    ...
    private async Task LoadSensorData()
    {
        try
        {
            ...
            if (data != null && data.Count > 0)
            {
                ...
                // 1. Update Mini Gauges directly from Sensor Data
                ...
                // 4. Request AI API Prediction to drive Main AQI Gauge & Labels
...
print("Updated DashboardPage.xaml.cs to be 100% driven by AI API Output!")
